...
function hideAllElements() {
    const elements = ["link", "offline", "error", "panel", "profil", "product", "rangliste", "nyaruhinweis", "banned", "loading", "rangliste2", "puscharecomfirm", "puschareerror"]
    ...
    return new Promise((resolve, reject) => {
        if (data !== null) {
            ...
            Twitch.ext.onAuthorized(auth => {
                ...
                fetch('https://api.nyaru.de/panel/update?location=twitch', { headers })
                    .then(response => response.json())
                    .then(data => {
                        cachedData = data; // Speichere die abgerufenen Daten
                        resolve(data);
                    })
                    .catch(reject);
            });
    ...
    Twitch.ext.onAuthorized(auth => {
        fetch(`https://api.nyaru.de/panel/purchases?product=${productIdToBuy}&location=twitch`, {
            ...
            .catch(
                () => {
                    changeSite('puschareerror');
                    document.getElementById('puschareerror-text').textContent = 'Unbekannter Fehler aufgetreten';
                    setTimeout(() => changeSite('panel'), 5000); // Redirect to panel after 2 seconds
    ...
    fetchData()
        .then(data => {
            ...
            if (product) {
                ...
                imgElement.src = 'https://api.nyaru.de/img/icons/error.png';
...
function loadRanking(offline) {

    fetchData()
        .then(data => {

            const table = document.getElementById(offline === 0 ? "ranglistetabele" : "ranglistetabele2")
...
function initialize() {
    ...
    document.getElementById('exit4').addEventListener('click', function() { changeSite('offline'); });
    ...
    document.getElementById('ranglisteiconne').addEventListener('click', function() { changeSite('rangliste2'); });
    ...
    Twitch.ext.onAuthorized(auth => {
        ...
        setInterval(() => {
            fetchData(null, true).then(data => {
                ...
                const isElementVisible2 = document.getElementById('rangliste2').style.display === 'block';
                if (status === 'online' && !isElementVisible) {
                    changeSite('panel'); // Show panel if the status is online and no other site is visible
                } else if (status === 'offline' && !isElementVisible2) {
