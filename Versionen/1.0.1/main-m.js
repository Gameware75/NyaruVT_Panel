...
function loadProducts(category, page = 1, data = cachedData) {
    ...
    const start = (page - 1) * (category === 'socken' ? 3 : 6);
    const largebild = (category === 'socken' || category === 'Ganzes Outfit');
    const end = start + (largebild ? 3 : 6);
    ...
    products.slice(start, end).forEach((product, index) => {
        ...
        productElement.innerHTML = `
            <img src="${product.img}" alt="${product.name}" style="width: 75px; height: ${largebild ? 150 : 75}px; object-fit: contain; background-color: rgba(255,255,255,0.53)" id="productimg ${index}" title="${product.name} (${product.price} Dias)">
        `;
    ...
    document.getElementById('next').disabled = start + (category === 'socken' ? 3 : 6) >= products.length;
